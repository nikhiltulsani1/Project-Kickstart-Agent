...
def generate_ci_workflow(intent: dict, copilot_client) -> str:
    ...
    system = (
        ...
        "- Install dependencies with this exact multi-line run step:\n"
        "    if [ -f requirements.txt ]; then pip install -r requirements.txt; fi\n"
        "    pip install pytest\n"
        "  Use that exact shell conditional so CI never fails if requirements.txt is absent.\n"
        "- Run tests with: pytest tests/test_scaffold.py\n"
        "  (target only the scaffold test — other test files may import stack packages not yet installed)\n"
        "- Set continue-on-error: false on the pytest step\n"
